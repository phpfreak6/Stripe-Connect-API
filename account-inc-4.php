...
$ssn_last_4	   		= get_param($userinfo,	'ssn_last_4');
...
$pstate			    = get_param($userinfo,	'pstate');
...
$personal_id_number = get_param($userinfo,	'personal_id_number');
...
$pid = false;
if(in_array('personal_id_number',$p)){
	$pid = true;
}
...
if(!empty($submit_form))
{
	...
	$ssn_last_4	   		= get_param($_POST,	'ssn_last_4');
	...
	$personal_id_number = get_param($_POST,	'personal_id_number');
	...
	if(!empty($personal_id_number)){ $account->legal_entity->personal_id_number = $personal_id_number; }
		if(!empty($accounttype)){ $account->legal_entity->type = $accounttype; }
		if(!empty($ssn_last_4)){ $account->legal_entity->ssn_last_4 = $ssn_last_4; }
		...
		if(!empty($paddress2)){ $account->legal_entity->personal_address->line2 = $paddress2; }
		if(!empty($ppostal_code)){ $account->legal_entity->personal_address->postal_code = $ppostal_code;	 }	
		if(!empty($pstate)){ $account->legal_entity->personal_address->state = $pstate; }
		...
		if(!empty($address2)){ $account->legal_entity->address->line2 = $address2; }
		if(!empty($postal_code)){ $account->legal_entity->address->postal_code = $postal_code;	 }	
		if(!empty($pstate)){ $account->legal_entity->address->state = $pstate;	 }	
		$account->save();
	}
	catch(Exception $e) {
